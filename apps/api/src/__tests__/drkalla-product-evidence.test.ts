...
describe('DrKalla product evidence lookup', () => {
  it('exposes price, brand, kind, and link facts per product', () => {
    ...
    expect(evidence?.priceText).toBe('9 Euro neunundneunzig');
  ...
  it('formats multi-variant prices as a German range', () => {
    expect(lookup.byId('lattafa-fakhar')?.priceText).toBe('von 24 Euro neunundneunzig bis 39 Euro neunundneunzig');
  ...
  it('keeps the evidence directive line compact and URL-free', () => {
    ...
    expect(line).toContain('9 Euro neunundneunzig');
    expect(line).not.toContain('9 Euro 99');
