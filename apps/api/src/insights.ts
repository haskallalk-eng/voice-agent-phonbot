...
async function processIssue(
  ...
  if (bestMatch && bestSimilarity >= SIMILARITY_THRESHOLD) {
    ...
    if (newCount >= autoApplyThreshold && cooldownOk && !appliedInThisCycle.applied) {
      appliedInThisCycle.applied = true;
      const optimizedFix = await generateOptimizedFix(examples, currentPrompt, ctx);

      const useAbTest =
        currentAvgScore != null &&
        currentAvgScore >= AB_TEST_SCORE_THRESHOLD &&
        !(await isAbTestRunning(orgId));

      if (useAbTest) {
        // Agent is performing well → A/B test the fix before committing permanently.
        // Apply variant prompt now, track next AB_TEST_CALLS_TARGET calls, auto-promote or rollback.
        const variantPrompt = currentPrompt ? `${currentPrompt}\n\n${optimizedFix}` : optimizedFix;
        await setPrompt(orgId, variantPrompt, 'ab_test_variant');
        await startAbTest(orgId, bestMatch.id, variantPrompt, currentPrompt);
      } else {
        // Agent needs improvement urgently → apply directly
        await applyPromptAddition(orgId, optimizedFix);
      }

      await pool.query(
        `UPDATE prompt_suggestions SET status='auto_applied', applied_at=now(), suggested_addition=$1 WHERE id=$2`,
        [optimizedFix, bestMatch.id],
      );
    }
  } else {
...
export async function registerInsights(app: FastifyInstance): Promise<void> {
  ...
  app.post('/insights/suggestions/:id/apply', { onRequest: [app.authenticate] }, async (req, reply) => {
    ...
    await applyPromptAddition(orgId, res.rows[0].suggested_addition as string);
    await pool.query(`UPDATE prompt_suggestions SET status='applied', applied_at=now() WHERE id=$1`, [id]);
    return { ok: true };
  });
