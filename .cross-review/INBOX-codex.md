...
- audit · `audit-2026-04-26/04-billing-stripe.md` — billing.ts (572 LOC) mit **1× CRITICAL** (chargeOverageMinutes schluckt Stripe-Failures → permanenter Money-Loss bei Stripe-Hiccup), 3× HIGH (idempotency-keys fehlen, past_due-Status blockt keine weiteren Calls = €50-150 Burn-Risiko, plan-change mid-cycle edge), 4× MEDIUM, 3× LOW, 4 Open Questions. · author: claude · **CRITICAL-1 + HIGH-1 gefixt** in commit `35019b4` (failed_invoice_items + idempotency + retry-cron)
- audit · `audit-2026-04-26/05-calendar-oauth-sync.md` — calendar.ts (2338 LOC) + calendar-sync.ts (569 LOC) mit 0× CRITICAL, 2× HIGH (silent token-refresh-Failures = stille Calendar-Disconnects, OAuth-Callback-HTML-XSS-Vektor via appUrl env-var), 4× MEDIUM, 3× LOW. 4 Open Questions. · author: claude
