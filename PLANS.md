...
- [x] 2026-06-13: Ran a multi-agent transcript+code weakness hunt (6 perspectives, adversarial verification, prioritized build list) over the 12 real DrKalla transcripts and the full custom-runtime code, then built and verified the confirmed fixes. Note: the verifiers correctly established that all stored transcripts are the Retell-managed RAG agent, not the custom runtime, so they are proxies; the "9-18 hours" transcript finding was refuted (KB is 10-18 everywhere). Shipped fixes: (1) P1 contact grounding — new governed `drkalla-contact-facts.ts` injects canonical address/hours/email/anfahrt/profi facts into the custom-runtime directives with a verbatim-quote + abstain-on-missing rule, plus a deterministic grounded contact fallback, so the model can no longer invent hours/address; (2) P1 salon-vs-shop guard added to the system prompt; (3) P1 SMS link-choice — a bare "ja" to the two-option Profi offer re-asks which link, and "Profi-Zugang" sends the Profi link (not the product link); executor widened to product|profi; (4) directive budget now drops the optional Memory line before disabling a turn so grounded answers are never blocked; (5) caller utterances now go through the central `redactForPrompt` (address/IBAN/card/DOB redaction before the model); (6) evidence line no longer asserts live stock; (7) replaced a dead KB-audit hallucination-guard branch with a real check; (8) funnel drops a stale product when the caller switches product type; (9) broadened repeat-request phrasings. Verified exhaustively: API typecheck, full API suite 112 files / 1354 tests, 1000/1000 memory A/B (`memoryP95Ms` 0.1, 0 extra LLM/KB calls), KB audit 1000/1000. Deployed to the live server (existing Docker path, Retell + demo sync skipped) and confirmed the grounding fixes live: hours now answer "Montag bis Freitag von 10 bis 18 Uhr" (no invention), address "Silbersteinstraße 83, 12051 Berlin", salon question correctly declined, Profi grounded with an SMS-link offer — all consistent Sie form, all streamed. Deliberately deferred (documented, lower-impact P2): in-flight stream abort on barge-in, WS-transport stale-frame tests, post-generation no-repeat/du-Sie output guards (auto-repair is grammatically fragile; relying on the strong Sie system instruction + Sie-form deterministic fallbacks), populating the voice-latency-contract from the live WS, and replacing prompt-substring A/B assertions with behavior assertions. No phone number, Retell-KB, production DrKalla agent, Own-KB primary flag, runtime rollout flag, or live SMS flag changed.
- [x] 2026-06-13: Ran exhaustive multi-turn live conversation testing against the deployed canary (7 realistic German scenarios: product funnel+compare+SMS, repeated contact, inaudible recovery, goodbye take-back, Profi choice, product-type switch, silence reminders) and fixed the two real bugs it surfaced. (W1, P1) The model paraphrased the SMS offer as "per SMS sende/zusende?" rather than "schicken", so `SMS_OFFER_QUESTION` missed the caller's "ja" and the model then asked for the caller's mobile number; broadened the matcher to schick/send/zusend/zukomm stems and added a prompt rule to never ask for the phone number (SMS goes to the verified caller number automatically). (W2, P2) On a clear goodbye the model appended a new question; added a prompt rule to close cleanly. Both verified live after redeploy: the Profi confirm now returns the truthful deterministic not-wired answer (no number request, 1ms, no model call) and "tschüss" closes cleanly. Confirmed live still-good: hours 10-18, address grounded, repeated-hours allowed on re-ask, inaudible ladder context-anchored without hangup, product-type switch drops the stale product, silence reminders escalate then close. Verified: typecheck, full suite 1355 tests, A/B 1000/1000 (`memoryP95Ms` 0.1). Top remaining quality lever (known, deferred): product-name detection recall for brand-line/partial names ("Koleston Perfect", "Synthesis Color Cream") — currently the agent safely defers price to the website and narrows rather than hallucinating, but grounded price/link needs better recall. No phone number, Retell-KB, production agent, Own-KB primary flag, rollout flag, or live SMS flag changed.
- [x] 2026-06-13: Improved product-name detection recall for partial/brand-line spoken names — correctness-first. Added a token-subset model (distinctive tokens from the product's actual spoken name only, minus stop/shop tokens, requiring >=2 caller content tokens that are all present in the product). A real-snapshot probe (408 products) showed that auto-resolving a partial match to one SKU collides too often against messy catalog data (translation duplicates, shared product lines) — a wrong price/link risk — so the fuzzy path deliberately NEVER auto-resolves a specific product; it only drives a variant clarification ("Von Koleston Perfect gibt es mehrere Ausführungen. Welche meinen Sie?"). Specific products still resolve only via the exact unique-alias path (caller speaks the full product name). Generic phrases produced 0 false single-resolutions in the probe. This replaces brand+line dead-ends (previously generic discovery) with a targeted variant question, with zero new wrong-product risk. Verified: typecheck, full suite 112 files / 1359 tests, A/B 1000/1000 (`memoryP95Ms` 0.109), KB audit 1000/1000. Noted for catalog hygiene (separate from this change): the pre-existing exact-alias path can still mis-resolve on duplicated/shared catalog aliases — a data-quality cleanup, not a detector bug. No phone number, Retell-KB, production agent, Own-KB primary flag, rollout flag, or live SMS flag changed.
